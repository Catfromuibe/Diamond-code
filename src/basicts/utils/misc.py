...
# Must be set before the first CUDA/CuBLAS call, otherwise GEMM is not deterministic.
...
def seed_everything(torch_seed: int) -> None:
    """Set random seeds and CUDA determinism for python/numpy/torch.

    CUBLAS_WORKSPACE_CONFIG must be set before the first CUDA op, otherwise
    matmul/attention is not reproducible on CUDA >= 10.2.
    """
